...
export interface tb_site_sch_boardAttributes {
  code: number;
  site_code?: number;
  subject?: string;
  contents?: string;
...
export type tb_site_sch_boardPk = "code";
export type tb_site_sch_boardId = tb_site_sch_board[tb_site_sch_boardPk];
export type tb_site_sch_boardOptionalAttributes = "site_code" | "subject" | "contents" | "register_code" | "reg_dt" | "updater_code" | "update_dt";
export type tb_site_sch_boardCreationAttributes = Optional<tb_site_sch_boardAttributes, tb_site_sch_boardOptionalAttributes>;

export class tb_site_sch_board extends Model<tb_site_sch_boardAttributes, tb_site_sch_boardCreationAttributes> implements tb_site_sch_boardAttributes {
  code!: number;
  site_code?: number;
  subject?: string;
  contents?: string;
  ...
  // tb_site_sch_board belongsTo tb_account via register_code
  ...
  // tb_site_sch_board belongsTo tb_account via updater_code
  ...
  // tb_site_sch_board belongsTo tb_site via site_code
  ...
  static initModel(sequelize: Sequelize.Sequelize): typeof tb_site_sch_board {
    return tb_site_sch_board.init({
    code: {
      autoIncrement: true,
    ...
    sequelize,
    tableName: 'tb_site_sch_board',
    schema: 'public',
    timestamps: false,
    indexes: [
      {
        name: "tb_site_sch_board_idx",
        fields: [
          { name: "site_code" },
        ]
      },
      {
        name: "tb_site_sch_board_pkey",
        unique: true,
        fields: [
          { name: "code" },
        ]
