...
export interface tb_site_build_majorAttributes {
  code: number;
  site_code: number;
  view_text: string;
...
export type tb_site_build_majorPk = "code";
export type tb_site_build_majorId = tb_site_build_major[tb_site_build_majorPk];
export type tb_site_build_majorOptionalAttributes = "reg_dt" | "update_dt";
export type tb_site_build_majorCreationAttributes = Optional<tb_site_build_majorAttributes, tb_site_build_majorOptionalAttributes>;

export class tb_site_build_major extends Model<tb_site_build_majorAttributes, tb_site_build_majorCreationAttributes> implements tb_site_build_majorAttributes {
  code!: number;
  site_code!: number;
  view_text!: string;
  register_code!: number;
  ...
  // tb_site_build_major belongsTo tb_account via register_code
  ...
  // tb_site_build_major belongsTo tb_account via updater_code
  ...
  // tb_site_build_major belongsTo tb_site via site_code
  ...
  static initModel(sequelize: Sequelize.Sequelize): typeof tb_site_build_major {
    return tb_site_build_major.init({
    code: {
      autoIncrement: true,
    ...
    sequelize,
    tableName: 'tb_site_build_major',
    schema: 'public',
    timestamps: false,
    indexes: [
      {
        name: "tb_site_build_major_idx",
        fields: [
          { name: "site_code" },
        ]
      },
      {
        name: "tb_site_build_major_pkey",
        unique: true,
        fields: [
          { name: "code" },
        ]
